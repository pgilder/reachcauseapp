...
Template.AgenciesInsertForm.events({
	"submit": function(e, t) {
		...
		function submitAction(result, msg) {
			var agenciesInsertFormMode = "insert";
			if(!t.find("#form-cancel-button")) {
				switch(agenciesInsertFormMode) {
					case "insert": {
						$(e.target)[0].reset();
					}; break;
					...
					case "update": {
						...
						pageSession.set("agenciesInsertFormInfoMessage", message);
					}; break;
				}
			}
		...
		function errorAction(msg) {
			...
			pageSession.set("agenciesInsertFormErrorMessage", message);
		}
...
Template.AgenciesInsertForm.helpers({
	"infoMessage": function() {
		return pageSession.get("agenciesInsertFormInfoMessage");
	},
